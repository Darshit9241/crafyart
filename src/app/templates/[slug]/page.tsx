...
import CategoryPage from "@/src/components/AsyncComponent/CategoryPage";
import NewCategoryTest from "@/src/components/AsyncComponent/newCategoryTest/NewCategoryPage";
...
const getData = async (
...
): Promise<{ jsonString: CategoryDataApiResponse; data: any }> => {
  ...
  try {
    const data = {
      ...
      filter: {
        ...(searchParams.language !== undefined && {
          language: Array.isArray(searchParams.language)
            ? `[${searchParams.language
                .map((element: string) => `'${element.toString()}'`)
                .join(",")}]`
            : `['${searchParams.language}']`,
        }),
        ...(searchParams.style !== undefined && {
          style: Array.isArray(searchParams.style)
            ? `[${searchParams.style
                .map((element: string) => `'${element.toString()}'`)
                .join(",")}]`
            : `['${searchParams.style}']`,
        ...
        ...(searchParams.interest && {
          interest: Array.isArray(searchParams.interest)
            ? `[${searchParams.interest
                .map((element: number) => `'${element.toString()}'`)
                .join(",")}]`
            : `['${searchParams.interest.toString()}']`,
        }),
        ...(searchParams.religion && {
          religion: Array.isArray(searchParams.religion)
            ? `[${searchParams.religion
                .map((element: string) => `'${element.toString()}'`)
                .join(",")}]`
            : `['${searchParams.religion}']`,
        ...
        ...(searchParams.theme && {
          theme: Array.isArray(searchParams.theme)
            ? `[${searchParams.theme
                .map((element: number) => `'${element.toString()}'`)
                .join(",")}]`
            : `['${searchParams.theme.toString()}']`,
        }),
        ...(searchParams.orientation && {
          orientation: Array.isArray(searchParams.orientation)
            ? `[${searchParams.orientation
                .map((element: number) => `'${element.toString()}'`)
                .join(",")}]`
            : `['${searchParams.orientation.toString()}']`,
        }),
        ...(searchParams.size && {
        ...
        ...(searchParams.license && {
          is_premium: `${searchParams.license}`,
        }),
        ...(searchParams.animation && {
          animation: `${searchParams.animation}`,
        }),
        ...(searchParams.color && {
    ...
    const response = await axios.post(
      `${apiUrl}/templates/api/getCategories`,
      data
    );
...
export default async function page({ params, searchParams }: PropsType) {
  const { slug } = params;
  const categoryData = await getData(slug, searchParams);
 
  return (
    <>
      {categoryData.jsonString?.new_api ? (
        <NewCategoryTest
          categoryData={categoryData.jsonString}
          categoryId={slug}
          slugId={slug}
