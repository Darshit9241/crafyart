...
export default function CategoryPage(props: {
  ...
  useEffect(() => {
    ...
    if (id) {
      api
        .getCategoryData({
          cat_id: id === "invitation" ? "invitation" : (id as any),
          page: page,
        })
    ...
    <>
      <Box className="bg-[#F4F7FE] px-[10px] sm:px-[16px]">
        <CustomHead
          ...
          heading={props?.categoryData?.jsonString?.data?.meta_title}
          text={props?.categoryData?.jsonString?.data?.meta_desc}
        />

        <Box>
          <link
            rel="canonical"
            title={props?.categoryData?.jsonString?.data?.meta_title}
            href={`https://www.craftyartapp.com/${props.slugId}`}
          />
        </Box>

        <Box className="pt-[15px] gap-[50px] flex">
          <Breadcrumb
            data={[
              { name: "Home", path: "/" },
              { name: id, current: true },
            ]}
          />
        </Box>
          ...
          >
            ...
            >
              <Typography
                ...
                variant="h1"
              >
                {props?.categoryData?.jsonString?.data?.h1_tag}
              </Typography>
              ...
              >
                {props?.categoryData?.jsonString?.data?.short_desc}
              </Typography>
            </Box>
            <Box
              sx={{
      ...
      {!staticBox[id] && (
        <Box className="my-[50px] w-[80%] mx-auto px-[30px] max-sm:w-full">
          <h2 className="text-[26px]  max-sm:text-[23px] text-[#1C3048] font-semibold mb-3 text-center">
            {props?.categoryData?.jsonString?.data?.h2_tag}
          </h2>

          <Typography className="text-[15px] whitespace-pre-line text-justify">
            {props?.categoryData?.jsonString?.data?.long_desc}
          </Typography>
        </Box>
      )}
      {staticBox[id]}
