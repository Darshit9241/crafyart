...
import SubCategoryDataBox from "./components/SubCategoryDataBox";
...
export default function NewCategoryTest(props: {
  ...
  const [newFilter, setNewFilter] = useState<any>(false);
  ...
  return (
    <>
      <Box className="bg-[#F4F7FE] px-[10px] sm:px-[16px]">
        ...
        />
        <Box>
        <link rel="canonical" title={props?.categoryData?.data?.seo_data?.meta_title} href={`https://www.craftyartapp.com/${props.slugId}`} />
        </Box>
        ...
        <Box sx={{ minHeight: `${screenHeight - 100}px` }}>
          ...
          <SubCategoryDataBox data={props?.categoryData?.data?.sub_category} />
